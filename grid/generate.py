...
def parse():
    ...
    parser.add_argument("--shapes", type=int, default=2, help="num different key+lock types")
    ...
    parser.add_argument("--prob-goal", type=float, default=1.0, help="probability of any key being mentioned in the goal")
...
def main():
    ...
    key_shapes = [shapes[k] if k < len(shapes) else random.choice(shapes) for k in range(args.keys)]   
    locked_shapes = [shapes[k] if k < len(shapes) else random.choice(shapes) for k in range(len(locked_positions))]
