...
def generate_for_solver(instance_name, num_decks, num_suits, num_values, s):
    ...
    for card_stack in  piles:
        lines.append(" ".join([str(len(card_stack))] + ["%s %s %s" % card for card in card_stack]))
...
def generate_readable(instance_name, num_decks, num_suits, num_values, s):
    lines = []
    deals, piles = s
    lines += ["Using %s decks of cards with %s suits per deck and %s values per suit" % (num_decks, num_suits, num_values)]
    lines += ["", "Deals"]
    deck = [c for deal in deals for c in deal]
    for i, deal in enumerate(deals):
        lines += ["deal %s: " % i +
                  " ".join(["d%s-s%s-v%s" % (deck, suite, value) for deck, suite, value in deal])]
    lines += ["", "Initial configuration of piles"]
    for i, pile in enumerate(piles):
        lines += ["pile %s: " % i +
                  " ".join(["d%s-s%s-v%s" % (deck, suite, value) for deck, suite, value in pile])]
    return lines
...
def generate_for_pddl(instance_name, num_decks, num_suits, num_values, s):
    ...
    objects = []
    objects += ["card-d%s-s%s-v%s - card" % (deck, suit, value)
                for deck in range(num_decks)
                for suit in range(num_suits)
                for value in range(num_values)]
    objects += ["pile-%s - tableau" % pile_id
                for pile_id in range(len(piles))]
    objects += ["deal-%s - deal" % deal_id
                for deal_id in range(len(deals) + 1)]

    facts = []
    # Cards in tableau piles
    for pile_id, pile in enumerate(piles):
        facts += ["(on card-d%s-s%s-v%s card-d%s-s%s-v%s)" % (card1 + card2)
        ...
        facts += ["(part-of-tableau card-d%s-s%s-v%s pile-%s)" % (card + (pile_id,))
                    for card in pile]
        pile_parts = split_pile_in_movable_parts(pile)
        facts += ["(movable card-d%s-s%s-v%s)" % card
                    for card in pile_parts[-1]]
        facts += ["(in-play card-d%s-s%s-v%s)" % card
                    for card in pile]
    ...
    for deal_id, deal in enumerate(deals):
        ...
        facts += ["(on card-d%s-s%s-v%s card-d%s-s%s-v%s)" % (card2 + card1)
                    for card1, card2 in zip(deal_deck[1:], deal_deck)][::-1]
        facts += ["(clear card-d%s-s%s-v%s)" % deal_deck[0]]

    facts += ["(current-deal deal-0)"]

    # Static predicates
    facts += ["(CAN-CONTINUE-GROUP card-d%s-s%s-v%s card-d%s-s%s-v%s)" % (deck1, suit, value, deck2, suit, value + 1)
                for deck1 in range(num_decks)
                for deck2 in range(num_decks)
                for suit in range(num_suits)
                for value in range(num_values - 1)]
    facts += ["(CAN-BE-PLACED-ON card-d%s-s%s-v%s card-d%s-s%s-v%s)" % (deck1, suit1, value, deck2, suit2, value + 1)
                for deck1 in range(num_decks)
                for deck2 in range(num_decks)
                for suit1 in range(num_suits)
                for suit2 in range(num_suits)
                for value in range(num_values - 1)]
    facts += ["(IS-ACE card-d%s-s%s-v0)" % (deck, suit)
                for deck in range(num_decks)
                for suit in range(num_suits)]
    facts += ["(IS-KING card-d%s-s%s-v%d)" % (deck, suit, num_values - 1)
                for deck in range(num_decks)
                for suit in range(num_suits)]
    facts += ["(NEXT-DEAL deal-%s deal-%s)" % (deal_id, deal_id + 1)
                for deal_id in range(len(deals))]
    for deal_id, deal in enumerate(deals):
        deal_deck = list(reversed(deal))
        facts += ["(TO-DEAL card-d%s-s%s-v%s pile-%s deal-%s deal-%s)" % (deal_deck[-1] + (len(deal) - 1, deal_id, deal_id))]
        facts += ["(TO-DEAL card-d%s-s%s-v%s pile-%s deal-%s card-d%s-s%s-v%s)" % (nextcard + (pile_id, deal_id) + card)
...
def random_state(num_decks, num_suits, num_values, num_piles, num_deals):
    ...
    piles = [cards[i * num_cards_per_pile:(i + 1) * num_cards_per_pile] for i in range(num_piles)]
...
def parse():
    ...
    parser.add_argument("num_decks", type=int, help="Number of decks of cards used for the game.")
    parser.add_argument("num_suits", type=int, help="Number of different suits of cards in each deck.")
    parser.add_argument("num_values", type=int, help="Number of different values of each suit in each deck.")
    parser.add_argument("num_piles", type=int, help="Number of piles.")
    parser.add_argument("num_deals", type=int, help="Number of deals (each deal contains one card for each pile, remaining cards are distributed to the piles).")
...
def main():
    ...
    instance_name = "spider-%s-%s-%s-%s-%s-%s" % (
        args.num_decks, args.num_suits, args.num_values, args.num_piles,
        args.num_deals, args.random_seed)

    random.seed(args.random_seed)
    s = random_state(args.num_decks, args.num_suits, args.num_values, args.num_piles, args.num_deals)
    if args.output_type == "raw":
        lines = generate_for_solver(instance_name, args.num_decks, args.num_suits, args.num_values, s)
    elif args.output_type == "pddl":
        lines = generate_for_pddl(instance_name, args.num_decks, args.num_suits, args.num_values, s)
    elif args.output_type == "readable":
        lines = generate_readable(instance_name, args.num_decks, args.num_suits, args.num_values, s)
    print ("\n".join(lines))
