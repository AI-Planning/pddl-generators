...
def main():
    ...
    i_grid =  [list(map(lambda x : max(0, int(x) - args.reduce_height_init),l.strip().split())) for l in args.init.readlines() ]
    g_grid =  [list(map(lambda x : max(0, int(x) - args.reduce_height_goal),l.strip().split())) for l in args.goal.readlines() ]
    ...
    if args.output_type.startswith("pddl"):
        lines = generate_for_pddl(instance_name, grid_size_x, grid_size_y, max_height, robots, depots, init_grid, goal_grid, args.multi_robot)
