...
parser.add_argument("-t", type=int, help="the number of trucks     (max:50)")
parser.add_argument("-l", type=int, help="the number of locations  (max:100)")
parser.add_argument("-p", type=int, help="the number of packages   (max:100)")
parser.add_argument("-a", type=int, help="the number of truck areas")

def main(args : argparse.Namespace, args2 : argparse.Namespace):

    with open(args.output, mode="w") as f:
        run(args,list(map(str,[rel("trucks"), "-seed", args.seed, *args.rest])),
            stdout=f)
