...
parser.add_argument("fridges", type=int, help="the number of fridges")
parser.add_argument("screws", type=int, help="the number of screws per backplane")


def main(args : argparse.Namespace, args2 : argparse.Namespace):

    with open(args.output, mode="w") as f:
        run(args,list(map(str,[rel("fridge"),
                               "-f", args2.fridge,
                               "-s", args2.screws,
                               "-r", args.seed])),
            stdout=f)
