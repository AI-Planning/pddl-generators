...
def print_pddl(instance_name, num_floors, elevators, passengers, cost_slow, cost_fast):
    ...
    above_facts = "\n       ".join([f"(above {count_objects[i]} {count_objects[j]})" for i in range (num_floors-1) for j in range(i+1, num_floors)])
    ...
    fast_elevator_objects = " ".join([e.name for e in elevators if e.name.startswith("fast")])
    slow_elevator_objects = " ".join([e.name for e in elevators if e.name.startswith("slow")])
...
class Elevator:
    ...
    def get_init(self):
        can_hold = " ".join([f"(can-hold {self.name} n{i+1})" for i in range(self.capacity)])
        reachable_floor = " ".join([f"(reachable-floor {self.name} n{i})" for i in self.floors])
...
def main():
    ...
    elevators = [Elevator(f"fast{i}", args.fast_capacity, 0, floors, fast_step_size) for i in range(args.fast_elevators)] + \
                [Elevator(f"slow{j}-{i}",  args.slow_capacity, j*args.area_size, min(floors, (j+1)*(args.area_size) + 1), slow_step_size) for i in range(args.slow_elevators) for j in range(args.areas)]
