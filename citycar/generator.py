...
for i in range(row):
    for j in range(column-1):
        print '(same_line junction'+str(i)+'-'+str(j)+' junction'+str(i)+'-'+str(j+1)+')'
        print '(same_line junction'+str(i)+'-'+str(j+1)+' junction'+str(i)+'-'+str(j)+')'

# connected cells on the same column
for i in range(column):
    for j in range(row-1):
        print '(same_line junction'+str(j)+'-'+str(i)+' junction'+str(j+1)+'-'+str(i)+')'
        print '(same_line junction'+str(j+1)+'-'+str(i)+' junction'+str(j)+'-'+str(i)+')'

# make the diagonal connections
for i in range(row-1):
    for j in range(column-1):
        print '(diagonal junction'+str(i)+'-'+str(j)+' junction'+str(i+1)+'-'+str(j+1)+')'
        print '(diagonal junction'+str(i+1)+'-'+str(j+1)+' junction'+str(i)+'-'+str(j)+')'
        print '(diagonal junction'+str(i)+'-'+str(j+1)+' junction'+str(i+1)+'-'+str(j)+')'
        print '(diagonal junction'+str(i+1)+'-'+str(j)+' junction'+str(i)+'-'+str(j+1)+')'
