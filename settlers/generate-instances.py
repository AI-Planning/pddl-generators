...
def generate(
    ...
    problem = (
        "p-{locations}-{seas}-{goals}-{constrainedness}-{vehicles}-{seed}.pddl".format(
            **locals()
        )
    )
...
def write_instance(folder, inst, map_name, const, vconst, name):
    problem = "instances-{}/p{}-{}-{:02d}-{}.pddl".format(
...
maps = {"tiny": (3, 2, 0), "small": (5, 5, 1), "large": (8, 9, 1), "huge": (16, 18, 2)}
...
resource_constrainedness = {"sat": (1.5, 5, 1.5, 3), "opt": (1.2, 4, 1.2, 2)}
    ...
    for goals in goal_range:
        ...
        while len(instances) < 100:
            instances = [
                ...
                for i in range(NUM_INSTANCES_GEN)
            ]
        ...
        instances = sorted(
            instances, key=lambda x: (x.get_resources(), x.get_vehicles())
        )
        ...
        for hardness in selection:
            write_instance(
                folder,
                instances[hardness_instance[hardness]],
                map_name,
                const,
                vconst,
                hardness,
