...
class Task(object):
  ...
  def _dump_header(self, out=None):
    print("; child-snack task with %s children and %s gluten factor " % (self.n_children, self.gluten_factor), file=out)
  ...
  def _dump_init(self, indent="", out=None):
    ...
    for c in self.children:
      print(indent + "%s (waiting %s %s)" % (indent, c, random.choice(self.tables)), file=out)
    ...
    if self.type == "pool":
      ...
      for t in self.trays:
        print("%s   (= (nsandwiches-ontray-no_gluten %s)  0)" % (indent,t), file=out)
