...
def numVarMatrix (predicate, objlist1, objlist2, indent="\n     "):
    numvars = [naryNumvar(predicate, list(obtuple)) for obtuple in itertools.product(objlist1, objlist2)]
...
def get_init():
    ...
    for i in range(1,num_ints+1):
        for j in range(1,i+1):
            strinit += indent + "(NUM_SUBSTRACT num"+str(i)+" num"+str(j) + " num"+str(i-j)+")"
    ...
    for k in range(2, num_workers + 1):
        ...
        strinit += indent + "(FOOD_REQUIRED worker"+str(k) + " num" + str((2 * k)+1) + ")"
